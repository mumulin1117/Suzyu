...
class SuzyMessageListCell: UITableViewCell {
    ...
    private func setupUI() {
        ...
        clipboardIcon.image = SuzyArtToyVibeEngine.suzyfilterApplySuzy(assetManagerSuue: "phone.circle")
    ...
    func configure(with model: SuzynoisetionModel) {
        avatarImageView.image = SuzyArtToyVibeEngine.suzyfilterApplySuzy(assetManagerSuue: (model.userInfo.suzyGenderSuzy == 0 ? "respectfulSpaceSuzy" : "confidentUserSuue"))
        nameLabel.text = model.userInfo.suzyUsernameSuzy
        timeLabel.text = model.lastTime
        
        if case .clipboard = model.noiseloi.last?.content {
            clipboardIcon.isHidden = false
            lastMsgLabel.isHidden = true
        } else {
            clipboardIcon.isHidden = true
            lastMsgLabel.isHidden = false
            if case .text(let t) = model.noiseloi.last?.content{ lastMsgLabel.text = t }
