//
//  SuzymagneticDetailViewController.swift
//  SuzyueBiag
//
...
class SuzymagneticDetailViewController: UIViewController {
    ...
    private func setupNavigation() {
        ...
        suzyCameraSwitchBtnSuzy.setImage(SuzyArtToyVibeEngine.suzyfilterApplySuzy(assetManagerSuue: "calcaremaifl"), for: .normal)
    ...
    private func setupTableView() {
        ...
        suzyChatTableSuzy.register(SuzymagneticBubbleCell.self, forCellReuseIdentifier: "BubbleCell")
    ...
    @objc private func sendAction() {
        guard let text = textField.text, !text.isEmpty else {
            SuzyHudManagerSuzy.shared.suzyeuncryptionSaltSuzy(filterApplySuzy: SuzyArtToyVibeEngine.suzyredoStackSuue(stateMachineSuzy: "TtzGbekkqYWULNQFmmuv847Twpd28s1LGiRW3PgV3K4ZTrq6kQfUsCS7Huk5WlCS/GlP1S2v0AufNgoWG7ldIQ=="), AdjustSuue: false)
            return
        }
      
        let newMessage = SuzyChatDetailRecord(isMine: true, content: .text(text), time: SuzyArtToyVibeEngine.suzyredoStackSuue(stateMachineSuzy: "lu3RRG7BBqbR1rRJtMGHqhx8D9QCOCnG+AH4gKnoBGrrw3s="))
    ...
    private func updateLocalCache(with msg: SuzyChatDetailRecord) {
        if let index = SuzynoiseCacheManager.shared.conversations.firstIndex(where: { $0.userInfo.suzyIdentifierSuzy == targetUser?.suzyIdentifierSuzy }) {
            SuzynoiseCacheManager.shared.conversations[index].noiseloi.append(msg)
        }else{
            guard let targetu = self.targetUser else {
                return
            }
            SuzynoiseCacheManager.shared.conversations.insert(SuzynoisetionModel.init(userInfo:targetu , unreadCount: 0, noiseloi: chatHistory), at: 0)
        }
    }
...
extension SuzymagneticDetailViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int { return chatHistory.count }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "BubbleCell", for: indexPath) as! SuzymagneticBubbleCell
